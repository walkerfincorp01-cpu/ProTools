...
import { ArrowRightLeft, Ruler, Maximize } from 'lucide-react';
...
const AREA_UNITS: Record<string, number> = {
  ...
  'Square Yard (sq yd)': 0.836127,
  'Acre': 4046.86,
  'Hectare': 10000,
  'Bigha (Standard)': 2529.28,
  'Guntha': 101.17,
  'Biswa': 126.46,
};
...
const UnitConverter: React.FC = () => {
  ...
  const currentUnits = mode === 'Length' ? LENGTH_UNITS : AREA_UNITS;
  ...
  const convert = (value: string, from: string, to: string) => {
    ...
    const fromInBase = val * currentUnits[from];
    const result = fromInBase / currentUnits[to];
    ...
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-10 border border-slate-100 shadow-xl max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
        <div className="flex flex-col gap-1 text-center sm:text-left">
          <h2 className="text-2xl font-black text-[#1E3A8A] dark:text-white flex items-center gap-3 justify-center sm:justify-start">
            ...
            Unit Converter (Jamin)
          </h2>
          <p className="text-slate-400 text-sm">Convert Length and Area units for land measurement.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-slate-50 dark:bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 relative">
        ...
        <div className="space-y-4">
          <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1">Convert From</label>
        ...
        {/* Swap Button (Absolute center on desktop, middle on mobile) */}
      ...
      {/* Common Conversions Quick Grid */}
      <div className="space-y-4 pt-4">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">Quick Conversions (Base: 1 {fromUnit})</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {Object.entries(currentUnits).slice(0, 4).map(([unit, factor]) => {
            const val = (parseFloat(fromValue) || 0) * (currentUnits[fromUnit] / factor);
            return (
